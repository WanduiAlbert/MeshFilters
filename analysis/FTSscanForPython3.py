...
def sinccorrection(self, datascans):
    ...
    minindex = list(map(lambda x: np.where(x >= 375)[0][0], datascans['encoder-driftcorrected']))
    maxindex = list(map(lambda x: np.where(x <= 400)[0][-1], datascans['encoder-driftcorrected']))
    # print(minindex, maxindex)
    # print (datascans['encoder'][0][minindex[0]])

    popts, pcovs = correction(datascans, sincfit, initialparams, minindex, maxindex)
    signalpeaks = np.array(list(map(lambda x: sincfit(-x[2]/x[1], *x), popts)))
    cosineshift = np.array(list(map(lambda x: np.cos((-x[2]/x[1])*x[-1]), popts)))
    phaseshift = np.round(list(map(lambda x: ((-x[2]/x[1])*x[-1])/(np.pi), popts)))
    encoderpeaks = np.array(list(map(lambda x, N: np.pi/x[-1]*N, popts, phaseshift)))[:,np.newaxis]

    self.signalpeaks = signalpeaks
    # encoderpeaks = np.array(map(lambda x:-x[2]/x[1], popts))[:,np.newaxis]
    relerr = np.array(list(map(lambda sigx, x: (sigx[2,2]/(x[2]**2) +
        sigx[1,1]/(x[1])**2 - 2*sigx[2,1]/(x[1]*x[2]))**0.5, pcovs, popts)))[:,np.newaxis]
    encoderpeakerr = encoderpeaks*relerr
    print("\n")
    for i in range(len(encoderpeaks)):
...
class FTSscan(object):
    def __init__(self, datadir, frequency, etalonlength,\
        useonearm=True, generateplots=True,\
    ...
    def symmetrize(self):
        ...
        # for i in range(len(self.sampledata['signal'])):
    ...
    def checkguesses(self):
        ...
        while (True):
            ...
            try: 
                response = input("Is the guess of the parameters acceptable? ")
